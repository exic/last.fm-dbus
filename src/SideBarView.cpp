...
SideBarTree::contextMenuEvent( QContextMenuEvent* e )
{
    ...
    if ( m_model->typeAtIndex( indexAt( e->pos())) != SideBar::MyProfile )
...
SideBarTree::contextMenuHandler( const QModelIndex& index, ContextMenuActionType the_action_to_do )
{
    ...
    SideBar::Type itemType = m_model->typeAtIndex( index );
    ...
    #define ICON_OF( index ) index.data( Qt::DecorationRole ).value<QIcon>()
    ...
    switch (itemType)
    {
        ...
        case SideBar::MyTags:
        {
            map[GoToTagsPage] = menu.addAction( ICON_OF( index ), tr("Go To My Tags Page") );
            menu.addSeparator();
        ...
        case SideBar::MyTagsChild:
        {
            ...
            map[PlayStation] = menu.addAction( ICON_OF( index ), tr("Play This Tag Station") );
            if( !The::user().settings().userTagsRadioHidden()) {
                map[PlayUserTagRadio] = menu.addAction( ICON_OF( index ), tr("Play Only Music You Tagged \"%1\"").arg( text ) );
            }
            ...
            if ( The::user().isSubscriber() && !The::user().settings().userTagsRadioHidden())
        ...
        case SideBar::NeighboursChild:
        {
            ...
            if (itemType == SideBar::FriendsChild)
            {
                menu.addSeparator();
                map[DeleteFriend] = menu.addAction( tr( "End Friendship" ) + ellipsis );
            }
            else if (itemType == SideBar::NeighboursChild)
            {
                menu.addSeparator();
                map[SortNeighboursBySimilarity] = menu.addAction( tr( "Sort By Similarity" ) );
                map[SortNeighboursBySimilarity]->setCheckable( true );
    ...
    switch (map.key( a ))
    {
        ...
        case PlayTrack:
            The::radio().play( m_model->trackAtIndex( index ) );
            break;

        case RecommendTrack:
        {
            The::shareDialog().setSong( m_model->trackAtIndex( index ) );
            The::shareDialog().exec();
        }
        break;

        case TagTrack:
            TagDialog( m_model->trackAtIndex( index ), this ).exec();
            break;
        ...
        case DeleteFriend:
            if (confirm( tr("Do you really want to remove %1 from your friends list?").barg( index.data().toString() ) ))
                (new DeleteFriendRequest( index.data().toString() ))->start();
            break;

        case Love:
            if (ConfirmDialog::love( m_model->trackAtIndex( index ), this ))
                (new LoveRequest( m_model->trackAtIndex( index ) ))->start();
            break;

        case UnLove:
            if (confirm( tr("Do you really want to remove %1 from your Loved Tracks?").barg( index.data().toString() ) ))
                (new UnLoveRequest( m_model->trackAtIndex( index ) ))->start();
            break;

        case Ban:
            if (ConfirmDialog::ban( m_model->trackAtIndex( index ), this ))
                (new BanRequest( m_model->trackAtIndex( index ) ))->start();
            break;

        case UnBan:
            if (confirm( tr( "Do you really want to un-ban %1?" ).arg( index.data().toString() ) ))
                (new UnBanRequest( m_model->trackAtIndex( index ) ))->start();
            break;

        case AddToPlaylist:
            if (ConfirmDialog::playlist( m_model->trackAtIndex( index ), this ))
                (new AddToMyPlaylistRequest( m_model->trackAtIndex( index ) ))->start();
            break;
...
bool
SideBarTree::dragDropHandlerPrivate( const QModelIndex& index, QDropEvent* event, QString &status )
{
...
//////
    switch (mimedata->itemType())
    {
        ...
        case UnicornEnums::ItemTrack:
            switch (m_model->typeAtIndex( index ))
            {
                case SideBar::FriendsChild:
                case SideBar::NeighboursChild:
                {
                    TrackInfo const track = mimedata->track();
                    QString const username = index.data().toString();

                    status = tr("Shares %1 with %2").barg( track.toString() ).barg( username );
                    if (is_drop && ConfirmDialog::share( track, username, this ))
                    {
                        RecommendRequest* r = new RecommendRequest( mimedata, username );
                        r->setLanguage( The::settings().appLanguage() );
                        r->start();
                    }
                    return true;
                }
    ...
    if (mimedata->hasTrack())
    {
        ...
        switch (m_model->typeAtIndex( index ))
        {
            ...
            case SideBar::LovedTracksRadio:
                ...
                if (is_drop && ConfirmDialog::love( track, this ))
            ...
            case SideBar::MyRecommendations:
                ...
                if (is_drop)
                {
                    RecommendRequest* r = new RecommendRequest( track, index.data().toString() );
                    r->setLanguage( The::settings().appLanguage() );
                    r->start();
                }
                return true;
    ...
    if (mimedata->hasUser())
    {
        if (m_model->classificationAtIndex( index ) == SideBarItem::Track)
        {
            Track const track = m_model->trackAtIndex( index );
            QString const username = mimedata->username();

            status = tr("Shares %1 with %2").barg( track ).barg( username );

            if (is_drop && ConfirmDialog::share( track, username, this ))
            {
                RecommendRequest* r = new RecommendRequest( track, username );
                r->setLanguage( The::settings().appLanguage() );
                r->start();
            }
            return true;
    ...
    if (mimedata->hasTag())
    {
        if (m_model->classificationAtIndex( index ) == SideBarItem::Track)
        {
            ...
            status = tr("Tag %1 as %2").barg( track ).barg( tag );

            if (is_drop && ConfirmDialog::tag( track, tag, this ))
            {
                SetTagRequest::append( track, tag );
            }
            return true;
        }
        else
...
SideBarTree::reset()
{
    ...
    setRowhidden( MyMixRadio, blank, true );
