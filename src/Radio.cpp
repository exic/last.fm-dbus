    ...
    if ( request->failed() )
    {
        if ( request->resultCode() == ChangeStation_InvalidSession )
        {
            reHandshake();        
             ...
             if (request->resultCode() == ChangeStation_FreeTrialExpired)
                 emit error( Radio_FreeTrialExpired, request->errorMessage() );
             else
                // Let Container deal with all the error conditions and display appropriate messages
...
{
    if (errorCode == Playlist_FreeTrialExpired)
    {
        emit error( Radio_FreeTrialExpired, message );
        return;
    }
