...
PlayCountsDatabase::exec( const QString& sql, const ITunesLibrary::Track& track )
{
    QSqlQuery query( m_db );
    query.prepare( sql );
    query.bindValue( ":path", track.path() );
...
{
    QSqlQuery query( m_db );
    query.prepare( "DELETE FROM itunes_db "
                        "WHERE path=:path" );
...
#ifdef Q_OS_MAC
    q.exec( "SELECT value FROM metadata WHERE key='schema'" );
    if (q.next() && q.value( 0 ).toString() != "2")
        return true;
#endif
...
AutomaticIPod::PlayCountsDatabase::bootstrap()
{
    ...
    query.exec( "DELETE FROM metadata WHERE key='schema'" );
...
#ifdef Q_OS_MAC
    query.exec( "INSERT INTO metadata (key, value) VALUES ('schema', '2')" );
#endif
